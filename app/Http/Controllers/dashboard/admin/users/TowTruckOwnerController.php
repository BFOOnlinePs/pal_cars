...
class TowTruckOwnerController extends Controller
{
    public function index()
    {
        $data = User::whereJsonContains('user_role', ['15'])->get();
        return view('dashboard.admin.users.tow_truck_owner.index', ['data' => $data]);
    }

    public function add()
    {
        return view('dashboard.admin.users.tow_truck_owner.add');
    }
    ...
    public function create(Request $request)
    {
        ...
        if ($data->save()) {
            return redirect()->route('dashboard.users.tow_truck_owner.index')->with(['success' => 'تم اضافة البيانات بنجاح']);
        } else {
    ...
    public function edit($id)
    {
        ...
        return view('dashboard.admin.users.tow_truck_owner.edit', ['data' => $data,'user_role'=>$user_role]);
    }
    ...
    public function update($id, Request $request)
    {
        ...
        $data->user_iban_number = $request->user_iban_number;
        if ($data->save()) {
            return redirect()->route('dashboard.users.tow_truck_owner.edit', ['id', $id])->with(['success' => 'تم اضافة البيانات بنجاح']);
        } else {
            return redirect()->back()->withInput();
        }
...
//        }
        return view('dashboard.admin.users.tow_truck_owner.details', ['data' => $data, 'company_contact_person' => $company_contact_person]);
    }
