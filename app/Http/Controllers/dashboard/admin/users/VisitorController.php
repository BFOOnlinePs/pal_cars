...
class VisitorController extends Controller
{
    ...
    {
        $data = User::whereJsonContains('user_role', ['16'])->get();
        return view('dashboard.admin.users.visitor.index', ['data' => $data]);
    }

    public function add()
    {
        return view('dashboard.admin.users.visitor.add');
    }
    ...
    public function create(Request $request)
    {
        ...
        if ($data->save()) {
            return redirect()->route('dashboard.users.visitor.index')->with(['success' => 'تم اضافة البيانات بنجاح']);
        } else {
    ...
    public function edit($id)
    {
        ...
        return view('dashboard.admin.users.visitor.edit', ['data' => $data,'user_role'=>$user_role]);
    }
    ...
    public function update($id, Request $request)
    {
        ...
        $data->user_iban_number = $request->user_iban_number;
        if ($data->save()) {
            return redirect()->route('dashboard.users.visitor.edit', ['id', $id])->with(['success' => 'تم اضافة البيانات بنجاح']);
        } else {
            return redirect()->back()->withInput();
        }
...
//        }
        return view('dashboard.admin.users.visitor.details', ['data' => $data, 'company_contact_person' => $company_contact_person]);
    }
